...
class AccountVatLedger(models.Model):
    ...
    def action_print(self):
        self.ensure_one()
        return self.env.ref("l10n_ar_fiscal_ws_reports.account_vat_ledger_xlsx").report_action(self)
    ...
    def _compute_files(self):
        for rec in self:
            rec._compute_one_file()

    def _compute_one_file(self):
        self.ensure_one()
        ...
        if self.REGINFO_CV_ALICUOTAS:
            ...
            self.aliquots_file = base64.b64encode(self.REGINFO_CV_ALICUOTAS.encode("ISO-8859-1"))
        else:
        ...
        if self.REGINFO_CV_COMPRAS_IMPORTACIONES:
            ...
            self.import_aliquots_file = base64.b64encode(self.REGINFO_CV_COMPRAS_IMPORTACIONES.encode("ISO-8859-1"))
        else:
        ...
        if self.REGINFO_CV_CBTE:
            ...
            self.vouchers_file = base64.b64encode(self.REGINFO_CV_CBTE.encode("ISO-8859-1"))
        else:
    ...
    def _get_REGINFO_CV_CBTE(self, alicuotas):
        ...
        for inv in invoices:
            ...
            if inv.currency_id != inv.company_currency_id and inv.invoice_currency_rate:
                # the helper no longer converts: this ledger always reported in company currency
                amounts = {key: value / inv.invoice_currency_rate for key, value in amounts.items()}
