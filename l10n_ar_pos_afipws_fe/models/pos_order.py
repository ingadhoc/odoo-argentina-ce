...
class PosOrder(models.Model):
    ...
    afip_auth_code = fields.Char(
        ...
        related="account_move.afip_auth_code"
    )
    afip_auth_code_due = fields.Date(
        string='CAE/CAI/CAEA due Date',
        related="account_move.afip_auth_code_due"
    )
        ...
        for order_id in self:
            if order_id.account_move and order_id.account_move.afip_qr_code:
                barcode = self.env['ir.actions.report'].barcode(
                    'QR',
                    order_id.account_move.afip_qr_code,
                    width=180,
                    height=180
                )
    ...
    def _prepare_invoice_vals(self):
        ...
        invoice_ids = self.refunded_order_ids.mapped('account_move').filtered(
            ...
            x.move_type in ['out_invoice'] and
            x.journal_id.afip_ws and x.afip_auth_code
        )
