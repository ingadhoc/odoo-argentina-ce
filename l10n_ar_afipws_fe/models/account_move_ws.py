...
class AccountMove(models.Model):
    ...
    def wsfe_invoice_add_info(self, ws, invoice_info):
        if invoice_info['mipyme_fce']:
            ...
            ws.AgregarOpcional(
                opcional_id=2101,
                valor=self.partner_bank_id.acc_number)
        ...
        if invoice_info['CbteAsoc']:
            doc_number_parts = self._l10n_ar_get_document_number_parts(
                    invoice_info['CbteAsoc'].l10n_latam_document_number, invoice_info['CbteAsoc'].l10n_latam_document_type_id.code)
            ws.AgregarCmpAsoc(
    ...
    def base_map_invoice_info(self):
        ...
        invoice_info[
            'country'] = invoice_info['commercial_partner'].country_id
        ...
        invoice_info['ws_next_invoice_number'] = int(
            self.journal_id.get_pyafipws_last_invoice(self.l10n_latam_document_type_id)) + 1
        ...
        invoice_info['nro_doc'] = invoice_info[
            'partner_id_code'] and invoice_info['commercial_partner'].vat or "0"
        invoice_info['cbt_desde'] = invoice_info['cbt_hasta'] = invoice_info[
            'cbte_nro'] = invoice_info['ws_next_invoice_number']
        invoice_info['concepto'] = invoice_info[
            'tipo_expo'] = int(self.l10n_ar_afip_concept)

        invoice_info['fecha_cbte'] = self.invoice_date
        invoice_info['mipyme_fce'] = int(invoice_info['doc_afip_code']) in [201, 206, 211]
        invoice_info['fecha_venc_pago'] = None

        # due date only for concept "services" and mipyme_fce
        if invoice_info['concepto'] != 1 and int(invoice_info['doc_afip_code']) not in [202, 203, 207, 208, 212, 213] or invoice_info['mipyme_fce']:
            invoice_info[
                'fecha_venc_pago'] = self.invoice_date_due or self.invoice_date
        ...
        invoice_info['imp_tot_conc'] = str(
            "%.2f" % amounts['vat_untaxed_base_amount'])
        ...
        if self.l10n_latam_document_type_id.l10n_ar_letter == 'C':
            ...
            invoice_info['imp_neto'] = str(
                "%.2f" % amounts['vat_taxable_amount'])

        invoice_info['imp_iva'] = str("%.2f" % amounts['vat_amount'])
        invoice_info['imp_trib'] = str(
            "%.2f" % amounts['not_vat_taxes_amount'])
        invoice_info['imp_op_ex'] = str(
            "%.2f" % amounts['vat_exempt_base_amount'])
    ...
    def wsfe_map_invoice_info(self):
        ...
        if invoice_info['fecha_venc_pago']:
            invoice_info['fecha_venc_pago'] = invoice_info[
                'fecha_venc_pago'].strftime('%Y%m%d')
        if invoice_info['fecha_serv_desde']:
            invoice_info['fecha_serv_desde'] = invoice_info[
                'fecha_serv_desde'].strftime('%Y%m%d')
        if invoice_info['fecha_serv_hasta']:
            invoice_info['fecha_serv_hasta'] = invoice_info[
                'fecha_serv_hasta'].strftime('%Y%m%d')
    ...
    def wsbfe_map_invoice_info(self):
        ...
        invoice_info['imp_perc_mun'] = invoice_info[
            'amounts']['mun_perc_amount']
        invoice_info['imp_internos'] = invoice_info['amounts'][
            'intern_tax_amount'] + invoice_info['amounts']['other_taxes_amount']
        invoice_info['imp_perc'] = invoice_info['amounts'][
            'vat_perc_amount'] + invoice_info['amounts']['profits_perc_amount'] + invoice_info['amounts']['other_perc_amount']
    ...
    def wsfex_map_invoice_info(self):
        ...
        if not country:
            ...
            raise UserError(_(
                ...
                'Country: %s' % (
                    self.journal_id.afip_ws, country.name)))
        ...
        if self.invoice_incoterm_id:
            ...
            if self.invoice_payment_term_id:
                ...
                invoice_info[
                    'obs_comerciales'] = self.invoice_payment_term_id.name
            else:
                invoice_info['forma_pago'] = invoice_info[
            ...
            # date
            invoice_info['fecha_pago'] = datetime.strftime(self.invoice_date_due, '%Y%m%d') \
                if int(invoice_info['doc_afip_code']) == 19 and invoice_info['tipo_expo'] in [2, 4] and self.invoice_date_due else ''
            ...
            if invoice_info['nro_doc']:
                ...
                invoice_info[
                    'pais_dst_cmp'] = self.commercial_partner.country_id.l10n_ar_afip_code
        invoice_info['lines'] = self.invoice_map_info_lines()
    ...
    def wsmtxca_map_invoice_info(self):
        invoice_info = self.base_map_invoice_info()
        invoice_info['fecha_cbte'] = invoice_info[
            'fecha_cbte'].strftime('%Y%m%d')
        if invoice_info['fecha_venc_pago']:
            invoice_info['fecha_venc_pago'] = invoice_info[
                'fecha_venc_pago'].strftime('%Y%m%d')
        if invoice_info['fecha_serv_desde']:
    ...
    def invoice_map_info_lines(self):
        ...
        for line in self.invoice_line_ids.filtered(lambda x: not x.display_type):
            ...
            line_temp['bonif'] = line.discount and str(
                "%.2f" % (line_temp['precio'] * line_temp['qty'] - line_temp['importe'])) or None
