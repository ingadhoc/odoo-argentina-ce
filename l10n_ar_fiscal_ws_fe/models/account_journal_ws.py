...
class AccountJournalWs(models.Model):
    ...
    def get_fiscal_ws_post_invoice_numbers(self):
        for journal_id in self:
            msg = []
            arca_ws = journal_id.arcaws
            if not arca_ws:
                raise UserError(_("No ARCA WS selected on point of sale %s") % (journal_id.name))
            import pdb; pdb.set_trace()
            ret = getattr(self, "%s_get_pyafipws_last_invoice" % journal_id.arca_ws.code)()
